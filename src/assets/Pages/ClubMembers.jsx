...
import Loader from "../Components/Loader";
import { AuthContext } from "../Contexts/AuthContext";
import Swal from "sweetalert2"; // SwAl use korle alert dekhte sundor lage

const ClubMembers = () => {
  const axiosSecure = useAxiosSecure();
  const { user } = useContext(AuthContext);

  // TanStack Query logic with refetch
  const {
    data: members = [],
    isLoading,
    refetch,
  ...
  const handleRemoveMember = async (id) => {
    Swal.fire({
      title: "Are you sure?",
      text: "You want to remove this member from the club?",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          const res = await axiosSecure.delete(`/remove-member/${id}`);
          if (res.data.success) {
  ...
  return (
    <div className="min-h-screen text-black p-6 bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      <div className="max-w-7xl mx-auto p-8 bg-white/50 backdrop-blur-md rounded-3xl shadow-2xl border border-white/20 transition-transform duration-500 hover:shadow-3xl">
        <h1 className="text-3xl font-extrabold mb-6 text-indigo-700 flex items-center gap-3">
          <span>👥</span> Club Members
        </h1>
            ...
            <table className="w-full border rounded-xl overflow-hidden bg-white/70 backdrop-blur-sm shadow-inner">
              <thead className="bg-indigo-600 text-white">
                <tr>
                  ...
                  <th className="p-3 text-left">Tracking ID</th>
                  <th className="p-3 text-left">Joined At</th>
                  <th className="p-3 text-center">Action</th> {/* New Column */}
                </tr>
              </thead>
              <tbody>
                {members.map((member) => (
                  ...
                  >
                    <td className="p-3">{member.customerEmail}</td>
                    <td className="p-3">{member.clubName || "N/A"}</td>
                    <td className="p-3 text-sm break-all font-mono">
                      {member.transactionId}
                    </td>
                    <td className="p-3 font-semibold text-indigo-600">
                      {member.trackingId}
                    ...
                    <td className="p-3 text-center">
                      <button
                        ...
                        className="px-4 py-2 bg-red-500 text-white text-sm font-bold rounded-lg hover:bg-red-600 transition shadow-md hover:shadow-red-200 active:scale-95"
                      >
                        Remove
                      </button>
                    </td>
